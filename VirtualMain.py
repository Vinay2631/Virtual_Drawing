...
# drawing tools
path = r"C:\Users\Windows\Downloads\tools.png"
tools = cv2.imread(path)
		...
		for i in op.multi_hand_landmarks:
			...
			if curr_tool == "draw":
				...
				if index_raised(yi, y9):
					cv2.line(paintWindow, (prevx, prevy), (x, y), 0, thick)
					cv2.line(frm,(prevx , prevy),(x,y),0,thick)
					prevx, prevy = x, y
				...
				if index_raised(yi, y9):
					...
					if var_inits:
						cv2.line(paintWindow, (xii, yii), (x, y), 0, thick)
						var_inits = False
				...
				if index_raised(yi, y9):
					...
					if var_inits:
						cv2.rectangle(paintWindow, (xii, yii), (x, y), 0, thick)
						var_inits = False
				...
				if index_raised(yi, y9):
					...
					if var_inits:
						cv2.circle(paintWindow, (xii, yii), int(((xii-x)**2 + (yii-y)**2)**0.5), (0,255,0), thick)
						var_inits = False
				...
				if index_raised(yi, y9):
					...
					cv2.circle(paintWindow, (x, y), 30, (100,100,100), -1)
